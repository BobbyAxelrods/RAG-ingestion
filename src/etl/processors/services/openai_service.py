...
class OpenAIService:
    ...
    def __init__(
        ...
        # Offline if flag set or required config missing
        self.offline = offline or not (
            bool(getattr(embedding_config, "endpoint", None))
    ...
    def summarize_text(self, text: str, max_chars: int = 300, max_retries: int = 3) -> str:
        ...
        # Prefer vision client for chat if available, else use embedding client chat
        for attempt in range(max_retries):
            try:
                if self.vision_client:
                    ...
                    # Use the embeddings client for chat if a chat deployment is configured there
